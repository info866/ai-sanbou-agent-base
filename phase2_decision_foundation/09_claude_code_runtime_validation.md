...
| テスト | 対象 | 期待 | 結果 |
|--------|------|------|------|
| 正本ブロック | ５.フェーズ２ 要件定義書.md | exit 2 | **exit 2（ブロック成功）** |
...
Agent tool（model: haiku）で実行。catalog-checker.md 定義の5項目を検査。

| 検査項目 | 結果 |
|---------|------|
| item_id 連番確認（F-001〜F-038 欠番・重複なし） | **PASS** |
| 参照整合性（05引き継ぎ文書 → 02カタログ） | **PASS** |
| 比較グループ所属マップ整合（パート4、F-032〜F-038反映） | **PASS** |
| current_status 正規値チェック（全38件） | **PASS** |
| 必須フィールド欠落チェック（5フィールド全件） | **PASS** |
